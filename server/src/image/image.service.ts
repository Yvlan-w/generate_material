...
字段说明（按照 Seedream 官方提示词结构组织）：
- theme（主题内容）：图片的核心主题，如品牌宣传、团队风采、数据可视化等
- content（内容文案）：图片中需要展示的文字内容，如"风险管理"、"耐心布局"等
- style（画面风格）：画风定位，如专业稳重、现代简约、科技感、商务大气等
- colorTone（色调倾向）：色彩搭配方案，如蓝色、灰色、暖色调、冷色调等
- scene（图片场景）：画面环境，如办公室、会议室、金融中心等
- emotion（情感基调）：氛围感觉，如专业、温暖、活力、沉稳等
- targetAudience（目标受众）：受众特征，如高净值客户、保守型投资者、普通投资者等
- usage（使用场景）：图像用途，如朋友圈、公众号头图、海报、线下展架等
- size（图片尺寸）：比例或尺寸，如1:1、16:9、1080x1920等
- referenceImages（参考图片）：参考图片及借鉴方面（如风格、色调、构图）
- includedElements（包含元素）：必须包含的元素及使用位置（如左上角logo、底部居中标语）
- otherRequirements（其他需求）：无法归类的补充信息

收集策略：
1. 按照【风格定位 → 主体细节 → 环境动态 → 色彩元素】的顺序引导用户
2. 如果必填字段尚未齐全，只引导用户补充下一个缺失的必填字段
3. 如果必填字段已齐全但可选字段仍有缺失，主动提示用户是否补充（例如"为了让素材更贴合使用场景，是否可以告诉我...？"）
4. 对于文字内容（content），请用户用双引号标出需要显示的文字
5. 对于风格（style）和色调（colorTone），引导用户给出具体明确的描述
6. 对于 includedElements（包含元素），引导用户说明图片中必须包含哪些元素，以及每个元素的使用位置，对于文字元素也需要用双引号标出
7. 对于 referenceImages（参考图片），询问用户是否有参考图片可以上传，以及想在哪些方面借鉴
8. 注意倾听用户的其他需求，对于无法归类到特定字段的信息，完整记录到otherRequirements（其他需求）中
9. 不要一次问多个问题，一次只引导一个字段
10. 避免重复已收集的信息
...
请判断该图片是否合规，输出JSON格式：
{
  ...
  private async generatePrompts(needs: StructuredNeeds, temperature?: number): Promise<{ positivePrompt: string; negativePrompt: string }> {
    ...
    const promptGenerator = `根据以下需求，生成一张投资咨询行业营销素材图片的正、负向提示词。
...
生成要求：
1. positivePrompt（正向提示词）：采用一段连贯的自然语言根据已收集的需求尽可能按照以下维度清晰描述画面
   - 风格定位：明确画风（如专业稳重、现代简约、科技感等），使用精准的风格词
   - 主体细节：描述画面核心主体及其特征
   - 环境动态：描述场景环境、动作状态、光影效果
   - 色彩元素：描述色调、配色方案
   - 构图布局：描述画面构图方式（如居中、对称、留白等）
   - 若涉及尺寸（size）
   - 其他需求（otherRequirements）
   - 文字处理：需要显示的文字内容必须放在双引号中，确保文字清晰可辨
   硬性约束：
  Ⅰ. 用户明确提供的所有信息，必须全部体现在描述中，不得遗漏；
  Ⅱ. 用户没有给出的信息，严禁自行脑补、虚构任何元素；
  Ⅲ. 语言精简精准，杜绝重复词汇堆砌；
2. negativePrompt（负向提示词）：列出需要避免的元素，用逗号分隔，包括：
   - 文字问题：文字扭曲、文字模糊、字体粘连、错字、乱码、畸形文字、残缺笔画、变形字符、杂乱水印
   - 违规元素：现金、金条、豪车、豪宅、陡峭上涨K线、涨停箭头、收益率数字、暴富、收益承诺、夸大宣传
   - 视觉问题：爆炸光、高饱和艳红色、夸张亢奋人物、低俗炫富元素、噪点严重、画面拥挤、无留白、画面元素过度堆砌、卡通风格、廉价感、杂乱背景
   - 比例问题：比例失调、人物变形

3. 正向提示词禁止出现任何违反投资咨询行业合规的文字（如收益承诺、夸大宣传、荐股等）。


输出严格 JSON 格式：
{
  "positivePrompt": "一段完整连贯的中文画面描述，覆盖风格、主体、环境、色彩、构图等维度",
...
直接输出摘要文本，不要加任何额外前缀。`;
  ...
  private buildFineTunePrompt(needs: StructuredNeeds, userFeedback: string): string {
    ...
    // 参考图片提示
    promptParts.push('参考图片：原图借鉴方面为整体风格、构图、姿态、元素位置。生成的图片风格、色调、构图应与参考图片保持一致');
    
    // 保留约束（简洁明确）
    promptParts.push('保留原图的核心主体和整体布局，仅对指定部分进行调整');
    
    // 风格与色调约束
    if (needs.style) promptParts.push(`保持${needs.style}风格`);
    if (needs.colorTone) promptParts.push(`保持${needs.colorTone}色调`);
    ...
    if (contextParts.length > 0) {
      promptParts.push(`背景参考：${contextParts.join('；')}`);
    }
